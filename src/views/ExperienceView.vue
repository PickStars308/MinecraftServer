<template>
  <div class="gallery-view">
    ...
    <p class="subtitle">回顾服务器的成长历程，见证每一个重要时刻</p>
    ...
    <div v-else-if="list.length === 0" class="empty-message">
      暂无记录
    </div>

    <div v-else class="masonry-container">
      <div v-for="(item, idx) in list" :key="idx" class="masonry-card" @click="openPreview(idx)">
      ...
      <div class="preview-wrapper">
        <div class="preview-image-container">
          ...
          <img
              ...
              :src="Server + currentPreviewItem?.cover"
        ...
        <div class="preview-toolbar">
          ...
          <button :disabled="previewIndex === list.length - 1" class="toolbar-btn" @click="nextImage">
            下一张
          </button>
        </div>
      </div>
...
import {getImageList} from '@/api/historyApi'
...
const currentPreviewItem = computed(() => {
  return previewIndex.value !== -1 ? list.value[previewIndex.value] : null
})
...
const fetchTimeline = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      errorMsg.value = res.message || '历程加载失败'
      addToast(errorMsg.value, 'error')
    }
  } catch {
    errorMsg.value = '历程加载失败'
    addToast(errorMsg.value, 'error')
  } finally {
    loading.value = false
  }
...
const nextImage = () => {
  if (previewIndex.value < list.value.length - 1) {
    previewIndex.value++
    scale.value = 1
    previewLoading.value = true
  }
